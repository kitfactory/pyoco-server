...
PYOCO_YAML_SCHEDULES_KV_BUCKET="pyoco_yaml_schedules"
...
# YAML schedule dispatch loop のポーリング間隔（秒）。
PYOCO_SCHEDULE_POLL_INTERVAL_SEC="1.0"
...
HTTP Gateway と managed worker を同時起動したい場合：

```bash
uv run pyoco-server up --with-nats-bootstrap --with-worker --worker-id local-1 --worker-tags default --worker-flow-resolver examples/hello_flow.py:resolve_flow --host 127.0.0.1 --port 8000
```

- `--with-worker` は managed worker を 1 プロセスだけ同時起動します。
- managed worker に渡せる追加引数は現状 `--worker-id` / `--worker-tags` / `--worker-flow-resolver` / `--worker-poll-timeout` のみです。
- YAML 実行だけを使う場合は `--worker-flow-resolver` を省略してよいです。`POST /runs` の Python flow 解決を行う場合は resolver を指定してください。
...
## 8) YAML schedule 実行
HTTP Gateway は `POST /schedules/yaml` / `GET /schedules` / `DELETE /schedules/{schedule_id}` で YAML schedule を扱えます。

- schedule 定義の保存先：`PYOCO_YAML_SCHEDULES_KV_BUCKET`
- dispatch loop 間隔：`PYOCO_SCHEDULE_POLL_INTERVAL_SEC`
- 対応種別：
  - one-shot：`run_at`（timezone offset 付き ISO 8601）
  - interval：`interval_seconds` + 任意 `start_at`
- schedule 作成時の YAML 検証・サイズ上限・`flow.defaults` の扱いは `POST /runs/yaml` と同一です。

CLI 例：
```bash
uv run pyoco-client --server http://127.0.0.1:8000 schedule-yaml --workflow-file ./flow.yaml --flow-name train --tag cpu --run-at 2026-04-02T10:00:00+09:00
uv run pyoco-client --server http://127.0.0.1:8000 schedule-yaml --workflow-file ./flow.yaml --flow-name train --tag cpu --interval-seconds 3600 --start-at 2026-04-02T10:00:00+09:00
uv run pyoco-client --server http://127.0.0.1:8000 schedules
```
